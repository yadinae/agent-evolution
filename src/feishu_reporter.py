...
class FeishuReporter:
    ...
    def check_report_for_anomalies(self, report_path: str, last_report_path: str = None) -> bool:
        """
        检查进化报告中是否包含**紧急**异常或重要事件
        
        通知策略（静默模式增强版）：
        - ✅ 发送：系统崩溃、数据丢失、API 配额耗尽、安全漏洞
        - ❌ 不发送：连续失败（已在 HEARTBEAT.md 记录）、历史遗留问题、P0 用户偏好
        
        Args:
            report_path: 当前进化报告文件路径
            last_report_path: 上次进化报告文件路径（用于对比，检测新增异常）
            
        Returns:
            bool: True 表示有紧急异常需要发送通知
        """
        if not Path(report_path).exists():
            return False
        ...
        # 只检测真正需要立即通知的紧急异常
        critical_anomaly_indicators = [
            # 系统崩溃/数据丢失
            r'系统崩溃 |System Crash|数据丢失|Data Loss',
            # API 配额耗尽
            r'API 配额耗尽|Quota Exhausted',
            # 安全漏洞
            r'安全漏洞 |Security Vulnerability|未授权访问',
        ]
        
        # 检查是否有紧急异常
        has_critical_anomaly = any(
            re.search(pattern, content, re.IGNORECASE)
            for pattern in critical_anomaly_indicators
        )
        
        # 注意：连续失败、P0 用户偏好等"慢性问题"不发送通知
        # 这些已在 HEARTBEAT.md 中记录，用户可随时查看
        
        if has_critical_anomaly:
            print("  🚨 检测到紧急异常 - 需要发送飞书通知")
        
        return has_critical_anomaly
    
    def _extract_consecutive_failures(self, content: str) -> dict:
        """
        从报告中提取连续失败信息
        ...
        # 匹配连续失败模式
        pattern = r'🚨\s*(.+?)\s*连续\s*(\d+)\s*天失败'
        matches = re.findall(pattern, content)
        
        for component, days in matches:
            component = component.strip()
            failures[component] = int(days)
